...
//https://github.com/mikeglueck/phenostacks

var utils = {
  getLeafNodes: function (node, attr) {
    var callback = function (n, p, d) {
      return (!n[attr] || n[attr].length === 0) ? n : null;
    };
  ...
  canCollapse: function (node) {
    ...
    if (node._children && utils.areAllLeafs(node._children)) {
      return false;
    }
    return true;
  },
...
TreeFilter.prototype._copyInternalNodes = function (node, excludeSelf) {
  ...
  excludeSelf = excludeSelf || false;
  var callback = function (n, p, d) {
    return (!n._leaf && !n._root) ? utils.copyNode(n) : null;
  };
  var result = utils.preorderDepthList(node, '_children', callback);
  return (excludeSelf) ? result.slice(1) : result;
}
...
TreeFilter.prototype._copyLeafNodes = function (node, excludeSelf) {
  ...
  excludeSelf = excludeSelf || false;
  var callback = function (n, p, d) {
    return (n._leaf) ? utils.copyNode(n) : null;
  };
  var result = utils.preorderDepthList(node, '_children', callback);
  return (excludeSelf) ? result.slice(1) : result;
}
...
TreeFilter.prototype._filterTree = function (node) {
  ...
  if (node._children && node._children.length > 0) {
    if (node._filters['collapsed'] && !node._filters['override']) {
      ...
      if (FILTEREMPTY) {
        var filtered = children.filter(function (d) {
          return !utils.allUnknown(d);
        });
        children = filtered;
      }
...
TreeFilter.prototype._clusters2 = function (data) {
  ...
  if (FILTEREMPTY) {
    leafNodes = leafNodes.filter(function (d) {
      return !utils.allUnknown(d);
    });
  }
  ...
  for (var hpid in leafPaths) {
    ...
    leafPaths[hpid].forEach(function (d) {
      console.log(d, sumArray(d.map(function (d) {
        return d.ic;
      })));
    });
  }
...
TreeFilter.prototype._clusters = function (data) {
  ...
  for (var leafId in leafAncestors) {
    ...
    if (ancestors.length > 0) {
      var cands = argmax(ancestors, function (n) {
        return n.attr.ic_base;
      }).map(function (d) {
        return ancestors[d];
      });
      //console.log(cands);
  ...
  function icLeafMax(node) {
    ...
    if (l.length > 0) {
      return d3.max(l.map(function (n) {
        return n.attr.ic_base;
      }))
    } else {
  ...
  function icLeafAvg(node) {
    var l = uniqueValuesObject(context._getLeafNodes(node), 'id');
    if (l.length > 0) {
      return averageArray(l.map(function (n) {
        return n.attr.ic_base;
      }))
    } else {
      return node.attr.ic_base;
  ...
  function icLeafMin(node) {
    var l = uniqueValuesObject(context._getLeafNodes(node), 'id');
    if (l.length > 0) {
      return d3.min(l.map(function (n) {
        return n.attr.ic_base;
      }))
    } else {
      return node.attr.ic_base;
  ...
  candidates = candidates.sort(function (a, b) {
    return d3.ascending(icLeafMin(a), icLeafMin(b)) || d3.ascending(a.attr.ic_base, b.attr.ic_base);
  });
  for (var i = 0, n = candidates.length; i < n; i++) {
    ...
    if (l.length > 0) {
      ...
      // Update max ic
      cluster.attr.ic_max = d3.max(cluster.children.map(function (d) {
        return d.attr.ic_base;
      }));
    }
...
TreeFilter.prototype._init = function (data) {
  ...
  var init = function (n, p, d) {
    utils.initNode(n, p, d, childrenAttr);
  };
